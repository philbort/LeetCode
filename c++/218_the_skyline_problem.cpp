...
public:
    ...
    vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
        ...
        for(auto points: buildings) {
            // Left edges
            edges.push_back(make_pair(points[0], -points[2]));
            // Right edges
            edges.push_back(make_pair(points[1], points[2]));
        }
        ...
        for (auto e: edges){
            if(e.second < 0)
            	// Insert the live edge
                m.insert(-e.second);
            else
            	// Delete the dead edge
                m.erase(m.find(e.second));
            // Get the last and largest value from the live edges
            cur = *(m.rbegin());
            // If it is a new height...
            if(cur != pre) {
                res.push_back(make_pair(e.first, cur));
                pre = cur;
            }
        }
